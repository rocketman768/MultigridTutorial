...
\begin{frame}{Gauss-Seidel Spectrum II}
 \begin{figure}
  \includegraphics[width=6cm]{images/jacobi_vs_gauss_eigs.pdf}
  \caption{It is obvious that $R_G$ decreases the error magnitude more quickly
           than $R_J$ at all frequencies.}
 \end{figure}
\end{frame}
...
\begin{frame}{Damped Gauss-Seidel}
 \begin{itemize}
  ...
  \item In contrast with $R_J(\omega)$, notice that we cannot significantly
        improve the high-frequency convergence, since
        $\mu_n^h(\omega) \approx 1$ regardless of $\omega$.
 \end{itemize}
\end{frame}
